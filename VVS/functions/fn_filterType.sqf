...
{
	case "All": {(VVS_Car + VVS_Helicopter + VVS_Plane + VVS_Maritime + VVS_Armored + VVS_Support + VVS_Autonomous)};
	case "Car": {VVS_Car};
	case "Helicopter": {VVS_Helicopter};
	case "Plane": {VVS_Plane};
	case "Maritime": {VVS_Maritime};
	case "Armored": {VVS_Armored};
	case "Autonomous": {VVS_Autonomous};
	case "Support": {VVS_Support};
	default {[]};
};
